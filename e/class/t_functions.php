...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	$firststr='<a title="Total record"><b>'.$num.'</b> 条记录</a>';
	//上一页
	if($page<>1)
	{
		$toppage='<li class="first-page"><a href="'.$dolink.$add['dofile'].$type.'">'.$fun_r['startpage'].'</a></li>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
        ...
        $toppage='<li class="first-page"><a>'.$fun_r['startpage'].'</a></li>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
			$prido=$add['dofile'].$type;
		...
		$prepage='<li class="prev-page"><a>'.$fun_r['pripage'].'</a></li>';
    }
	//下一页
	if($page!=$totalpage)
	{
		...
		$lastpage='<li class="last-page"><a href="'.$lastpagelink.'">'.$fun_r['lastpage'].'</a></li>';
	} else {
        $pagenex=$page+1;
		$nextpagelink=$repagenum&&$repagenum<$pagenex?eReturnRewritePageLink2($add,$pagenex):$dolink.$add['dofile'].'_'.$pagenex.$type;
		$lastpagelink=$repagenum&&$repagenum<$totalpage?eReturnRewritePageLink2($add,$totalpage):$dolink.$add['dofile'].'_'.$totalpage.$type;
		$nextpage='<li class="next-page"><a>'.$fun_r['nextpage'].'</a></li>';
		$lastpage='<li class="last-page"><a>'.$fun_r['lastpage'].'</a></li>';
    }
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1="<li class='current_page_num'>";
			$is_2="</li>";
		}
		elseif($i==1)
		{
...
function sys_GetSitelink($line,$num,$enews=0,$classid=0,$stats=0){
	...
	if($i<>0)
	{
		$table="<table width=100% border=0 cellpadding=3 cellspacing=0 class='table1'>";$table1="</table>";
        $ys=$line-$i%$line;
		$p=0;
        for($j=0;$j<$ys&&$ys!=$line;$j++)
		{
